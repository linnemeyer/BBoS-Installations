prompt --application/pages/page_10110
begin
--   Manifest
--     PAGE: 10110
--   Manifest End
wwv_flow_api.component_begin (
 p_version_yyyy_mm_dd=>'2021.10.15'
,p_release=>'21.2.6'
,p_default_workspace_id=>1608381483082323
,p_default_application_id=>100
,p_default_id_offset=>336536533326679440
,p_default_owner=>'BBOSINSTALLS'
);
wwv_flow_api.create_page(
 p_id=>10110
,p_user_interface_id=>wwv_flow_api.id(16247647600803137983)
,p_name=>'About this Application'
,p_alias=>'HELP'
,p_step_title=>'About this Application'
,p_first_item=>'AUTO_FIRST_ITEM'
,p_autocomplete_on_off=>'OFF'
,p_required_patch=>wwv_flow_api.id(64202836879542443)
,p_protection_level=>'C'
,p_help_text=>'All application help text can be accessed from this page. The links in the "Documentation" region give a much more in-depth explanation of the application''s features and functionality.'
,p_last_updated_by=>'LARRY'
,p_last_upd_yyyymmddhh24miss=>'20210714001739'
);
wwv_flow_api.create_page_plug(
 p_id=>wwv_flow_api.id(64203491270542460)
,p_plug_name=>'About Page'
,p_region_template_options=>'#DEFAULT#:t-ContentBlock--padded:t-ContentBlock--h1:t-ContentBlock--lightBG'
,p_plug_template=>wwv_flow_api.id(16247567115335137928)
,p_plug_display_sequence=>20
,p_plug_source=>'BB Installs &copy; 2021, was created for Budget Blinds of Scottsdale'
,p_plug_query_num_rows=>15
,p_plug_query_options=>'DERIVED_REPORT_COLUMNS'
,p_attribute_01=>'N'
,p_attribute_02=>'HTML'
,p_attribute_03=>'Y'
);
wwv_flow_api.component_end;
end;
/
